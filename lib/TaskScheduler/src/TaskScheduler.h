...
#endif
#else
// Non-Arduino (native) build: provide minimal stubs so TaskScheduler
// compiles without the Arduino framework.  Tasks with interval 0
// (TASK_IMMEDIATE) fire on every execute() pass regardless of the
// return value of millis(), so this stub is sufficient for tests.
#include <cstdint>
...
bool Scheduler::execute() {
    ...
    // If negative - the task is behind (next execution time is already in the past)
                ...
                nextTask = iCurrent->iNext;  // re-read: skips tasks deleted during callback (UAF fix)
#else
