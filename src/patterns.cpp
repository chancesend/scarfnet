...
PatternList getPatternList()
{
    ...
    patterns.push_back({"pride", [](Leds& leds, int32_t timeMs, CRGBPalette16 palette, Rnd rnd) {
        pride(leds, timeMs, palette);
    }});

    patterns.push_back({"confetti", [](Leds& leds, int32_t timeMs, CRGBPalette16 palette, Rnd rnd) {
    ...
    patterns.push_back({"firework", [](Leds& leds, int32_t timeMs, CRGBPalette16 palette, Rnd rnd) {
        int periodMs = rndRange(rnd, 10, 85) * 100;  // launch cycle length
        firework(leds, timeMs, periodMs, palette);
    }});

    patterns.push_back({"colorwaves", [](Leds& leds, int32_t timeMs, CRGBPalette16 palette, Rnd rnd) {
        colorwaves(leds, timeMs, palette);
    }});

    patterns.push_back({"cylon", [](Leds& leds, int32_t timeMs, CRGBPalette16 palette, Rnd rnd) {
